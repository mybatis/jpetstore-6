    ...
    node(kubelabel) {
        stage('cache check') {
            container('kubectl'){
                ...
                echo "${node}"
                def nodeoutput=sh returnStdout: true, script: "kubectl describe node ${node}"
                def zone = sh returnStdout: true, script: "echo ${nodeoutput} | grep ProviderID" // | sed -e 's/.*aws:\\/\\/\\///g' | sed -e 's/\\/.*//g'"
                echo "${zone}"
            }
        }
    }
