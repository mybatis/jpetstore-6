...
/**
 * JPetStore 스타일의 단순 Gemini 클라이언트. - 동기 호출 - 예외는 RuntimeException으로 단순 처리 - 반환값: JSON 문자열 (우리 프롬프트에서 강제한 구조)
 */
public class GeminiClient {
  ...
  /**
   * Gemini 응답 text에서 JSON 부분만 추출 - ```json ... ``` 코드블록 제거 - ``` ... ``` 코드블록 제거 - 코드블록 없으면 그대로 반환
   */
